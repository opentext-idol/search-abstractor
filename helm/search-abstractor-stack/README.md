...
| @opentext-otds | auth(otds) | 25.4.0 |
...
| https://raw.githubusercontent.com/opentext-idol/idol-containers-toolkit/main/helm | view(idol-view) | ~0.8.0 |
...
| auth.external.host | string | `""` | External hostname for OTDS instance (e.g. host.name) Should match auth.otdsws.publicHostname |
| auth.external.port | int | `80` | External port number of OTDS instance (e.g. 8080) |
...
| auth.otdsws.publicHostname | string | `""` | Public hostname for OTDS instance (e.g. host.name) |
...
| auth.users | object | `{"sample_user":{"passwordEnv":"SAMPLE_USER_PASSWORD","passwordTemporary":true}}` | set of users to create in OTDS partition on initialization if initEnabled is true |
...
| distributedidol.content.cfg.fieldprocessing | string | "" | Additional Content field processing configuration data |
| distributedidol.content.cfg.security | string | "" | Additional Content security configuration data |
...
| saapi.backendIdolAnswerServerHost | string | `"idol-answerserver"` | hostname for Answer Server component |
| saapi.backendIdolAnswerServerPort | string | `"12000"` | port for Answer Server component |
| saapi.backendIdolAnswerServerSystemOrdering | string | `"AnswerBank,FactBank,RAGPassageExtractor"` | system ordering for AnswerServer: ask questions of these systems in this order |
...
| saapi.backendIdolViewHost | string | `"idol-view"` | hostname for View Server component |
| saapi.backendIdolViewPort | string | `"9080"` | port for View Server component |
...
| saapi.image.version | string | `"26.3.0"` | The version value to use to construct the container image name: {registry}/{repo}:{version} |
...
| saapi.llama.apiKey | string | `"My API Key"` | The API key to use for the endpoint |
| saapi.llama.enabled | bool | `false` | Should we use a deployed LLaMA model for LLM support? |
| saapi.llama.endpointBase | string | `"http://llama-endpoint:8000/v1"` | Endpoint base for the deployed LLaMA model |
| saapi.llama.hfModel | string | `"my model"` | equivalent HuggingFace model name (used to obtain tokenizer) |
| saapi.llama.hfToken | string | `""` | HuggingFace token to access the model tokenizer. If empty, a cached tokenizer will be used if available |
| saapi.llama.model | string | `"my model"` | Name of the deployed LLaMA model to use |
| saapi.llama.modelRevision | string | `""` | The LLM revision to use (branch, tag, or commitid) |
| saapi.llama.multimodal | bool | `false` | Is the model multimodal (i.e. does it support image inputs?) If it is, it will also be used for image processing at ingest time |
...
| saapi.metadataPromptFile | string | `"resources/filters_mistral.json"` | Prompt file to use for LLM-based metadata filter extraction (used when rewriteQueries is true) |
...
| saapi.rewriteQueries | bool | `false` | Should query rewriting and metadata identification be enabled? |
...
| saapi.sessionLicensor.name | string | `"idol-answerserver"` | Session licensor name |
| saapi.sessionLicensor.port | int | `12000` | Port session licensor runs on |
...
| saapi.vertexai.maxOutputTokens | int | `1024` | Limit the number of output tokens returned by the Generative model (includes thinking tokens). Set to 0 for no limit (model default maximum). |
| saapi.vertexai.model | string | `"gemini-2.5-flash"` | Name of generative model to use with VertexAI |
...
| saapi.vllm.modelRevision | string | `""` | The LLM revision to use (branch, tag, or commitid) |
